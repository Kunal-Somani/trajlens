...
class TestTerminalRenderer:
    def _render(self, results: list[CheckResult]) -> str:
        ...
        render_terminal("test/ref", _FORMAT_ID, _FORMAT_VERSION, 5, 100, results, console=buf)
    ...
    def test_num_frames_none_shows_unknown(self) -> None:
        buf = Console(file=None, record=True, highlight=False, markup=False)
        render_terminal("test/ref", _FORMAT_ID, _FORMAT_VERSION, 5, None, _CLEAN, console=buf)
        out = buf.export_text()
...
class TestJsonRenderer:
    def test_clean_dataset_grade_pass(self) -> None:
        doc = json.loads(render_json("r", _FORMAT_ID, _FORMAT_VERSION, 1, 10, _CLEAN))
        assert doc["grade"] == "PASS"

    def test_fail_dataset_grade_fail(self) -> None:
        doc = json.loads(render_json("r", _FORMAT_ID, _FORMAT_VERSION, 1, 10, _WITH_FAIL))
        assert doc["grade"] == "FAIL"

    def test_warn_dataset_grade_warn(self) -> None:
        doc = json.loads(render_json("r", _FORMAT_ID, _FORMAT_VERSION, 1, 10, _WITH_WARN))
        assert doc["grade"] == "WARN"

    def test_error_dataset_grade_error(self) -> None:
        doc = json.loads(render_json("r", _FORMAT_ID, _FORMAT_VERSION, 1, 10, _WITH_ERROR))
    ...
    def test_schema_has_required_fields(self) -> None:
        doc = json.loads(render_json("my/ref", _FORMAT_ID, _FORMAT_VERSION, 3, 90, _CLEAN))
        assert doc["ref"] == "my/ref"
        assert doc["format_id"] == "lerobot"
    ...
    def test_num_frames_none_serializes(self) -> None:
        doc = json.loads(render_json("r", _FORMAT_ID, _FORMAT_VERSION, 1, None, _CLEAN))
        assert doc["num_frames"] is None


class TestHtmlRenderer:
    def test_clean_contains_pass(self) -> None:
        html = render_html("r", _FORMAT_ID, _FORMAT_VERSION, 1, 10, _CLEAN)
        assert "PASS" in html

    def test_fail_contains_fail(self) -> None:
        html = render_html("r", _FORMAT_ID, _FORMAT_VERSION, 1, 10, _WITH_FAIL)
        assert "FAIL" in html
    ...
    def test_ref_is_escaped(self) -> None:
        html = render_html("<script>alert(1)</script>", _FORMAT_ID, _FORMAT_VERSION, 1, 10, _CLEAN)
    ...
    def test_message_is_escaped(self) -> None:
        ...
        html = render_html("r", _FORMAT_ID, _FORMAT_VERSION, 1, 10, results)
        assert '<img src=x onerror="alert(1)">' not in html

    def test_num_frames_none_shows_unknown(self) -> None:
        html = render_html("r", _FORMAT_ID, _FORMAT_VERSION, 1, None, _CLEAN)
        assert "unknown" in html


class TestSarifRenderer:
    def _parse(self, results: list[CheckResult]) -> dict[str, object]:
        return json.loads(render_sarif("test/ref", _FORMAT_ID, _FORMAT_VERSION, 1, 10, results))  # type: ignore[return-value]
